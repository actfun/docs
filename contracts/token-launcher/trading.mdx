...
```solidity
function sellTokens(uint256 tokenAmount, uint256 minUsdcOut) external
```
...
<ParamField path="minUsdcOut" type="uint256" required>
  Minimum USDC (in wei) you are willing to receive. The transaction reverts with `"TokenLauncher: slippage exceeded"` if `usdcOut < minUsdcOut`. Pass `0` to skip slippage protection.
</ParamField>
...
const usdcIn = parseEther("1"); // 1 USDC

// For "I'm sending usdcIn, how many tokens do I get?" compute client-side
// using the AMM formula against the current pool reserves:
const [tokenReserve, usdcReserve] = await Promise.all([
  publicClient.readContract({ address: launcherAddress, abi: LAUNCHER_ABI, functionName: "tokenReserve" }),
  publicClient.readContract({ address: launcherAddress, abi: LAUNCHER_ABI, functionName: "usdcReserve" }),
]);

const tokensOut = (usdcIn * tokenReserve) / (usdcReserve + usdcIn);
...
const minUsdcOut = (usdcOut * 99n) / 100n; // 1% slippage
