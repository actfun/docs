...
After a token graduates from Phase 1, its `TokenLauncher` contract operates as a constant-product AMM (`x * y = k`). You can buy tokens by sending ARC via `buyTokens`, or sell tokens back for USDC via `sellTokens`. Both functions revert if you are not graduated yet, and both accept a slippage protection parameter so your transaction does not execute at a worse price than you expect.
...
Transfer tokens from your wallet to the launcher and receive ARC. You must approve the launcher contract to spend your tokens before calling this function.
...
<ParamField path="minArcOut" type="uint256" required>
  Minimum USDC (in wei) you are willing to receive. The transaction reverts with `"TokenLauncher: slippage exceeded"` if `arcOut < minArcOut`. Pass `0` to skip slippage protection.
</ParamField>
