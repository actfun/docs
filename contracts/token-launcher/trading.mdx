...
Send ARC as `msg.value`. The contract calculates how many tokens to send you using the constant-product formula and transfers them to your wallet. The call reverts if the output would be zero, fall below `minTokensOut`, or exceed the available token reserve.
