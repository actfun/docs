...
console.log(`Price: ${formatEther(price)} USDC per token`);
```
...
```solidity
function estimateBuy(uint256 tokensOut) external view returns (uint256 usdcIn)
```
...
```solidity
function estimateSell(uint256 tokensIn) external view returns (uint256 usdcOut)
```
...
console.log(`Buy  100 tokens: ${formatEther(usdcNeeded)} USDC`);
console.log(`Sell 100 tokens: ${formatEther(usdcReceived)} USDC`);
```
...
| `usdcReserve` | `uint256` | Current USDC balance of the AMM pool (wei). `0` before graduation. |
...
| `feePerMine` | `uint256` | USDC (wei) required per `mine()` call. Immutable. |
...
```typescript
const [graduated, tokenReserve, usdcReserve, mineableSupply] = await Promise.all([
  publicClient.readContract({
    address: launcherAddress,
    abi: LAUNCHER_ABI,
...
console.log({ graduated, tokenReserve, usdcReserve, mineableSupply });
```

<Note>
  `tokenReserve` and `usdcReserve` are `0` before graduation. Do not use `getTokenPrice`, `estimateBuy`, or `estimateSell` before `graduated == true` — `getTokenPrice` returns `0`, while `estimateBuy` and `estimateSell` will return `0` or compute against zero reserves.
</Note>
