...
3. Sets `usdcReserve = address(this).balance` (all accumulated mine fees minus any previously claimed refunds).
...
Emitted in the same transaction as the final mine call. `tokenReserve` and `usdcReserve` are the initial AMM reserves.
