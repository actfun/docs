...
**Contract:** `LaunchpadFactory` at `0xD3a684B4D9aA0E92E79ade7DcaB70A8b125A7a4B` on Arc Testnet (Chain ID 5042002)
...
<ParamField path="feePerMine" type="uint256" required>
  USDC (in wei) the caller must send with each `mine()` call. These fees accumulate in the `TokenLauncher` and seed the AMM liquidity pool on graduation. Set to `0` for free mining (though the factory's creation fee still applies).
</ParamField>
...
const hash = await walletClient.writeContract({
  ...
  args: [
    ...
    parseEther("0.01"),          // feePerMine: 0.01 USDC
  ],
  value: creationFee,
});
