...
No data is sent to any server to power alerts. Your mined token list lives entirely in your browser's `localStorage` and is never transmitted to ACTFUN's backend. The poller reads on-chain state directly from Arc Testnet via RPC.
