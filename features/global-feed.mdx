---
title: "Global Live Feed: real-time on-chain activity across all tokens"
sidebarTitle: "Global Live Feed"
description: "The MINEPAD Global Live Feed streams every mine, buy, sell, and graduation event across all tokens in real time, powered by Goldsky Turbo and Neon Postgres."
---

The **Global Live Feed** is a real-time activity stream on the MINEPAD homepage. It shows every on-chain event — mines, buys, sells, and graduations — across every token on the platform, as they happen.

<Info>
  The feed is powered by a **Goldsky Turbo** pipeline that streams Arc Testnet raw logs into a Neon Postgres database. The MINEPAD API server queries that database and serves events to the browser. A "Powered by Goldsky" badge appears in the feed footer.
</Info>
...
4. The MINEPAD API server (`GET /api/onchain-events`) queries the table and decodes events using `viem`.
