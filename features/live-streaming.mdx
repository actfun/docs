...
description: "Token creators can broadcast live video directly from the MINEPAD token page. Viewers watch via WebRTC peer-to-peer with a live chat panel and real-time viewer count."
---

Every token page on MINEPAD includes a built-in livestream studio for the token's creator. No streaming platform account, no OBS setup, no external tools — just click **Go Live** and your camera goes live to everyone watching the token page.
...
MINEPAD uses **WebRTC peer-to-peer** video to stream directly from the creator's browser to each viewer's browser. There is no central media server — the video travels directly between peers, which means near-zero latency and no streaming account required.
...
| Signalling | MINEPAD API server (WebSocket) |
