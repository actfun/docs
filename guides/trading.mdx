...
description: "Trade graduated ACTFUN tokens using the built-in constant-product AMM — send ARC to buy, or approve and sell tokens back for ARC."
---
...
MINEPAD uses a constant-product formula: `x * y = k`, where `x` is the ARC reserve, `y` is the token reserve, and `k` remains constant after each trade.

- **Buying** sends ARC in and receives tokens out: `tokensOut = arcIn × tokenReserve / (arcReserve + arcIn)`
- **Selling** sends tokens in and receives ARC out: `arcOut = tokensIn × arcReserve / (tokenReserve + tokensIn)`

The pool is seeded at graduation with the 5% LP reserve tokens and all ARC fees collected during mining. There are no additional fees on top of gas.
...
<Steps>
  <Step title="Open the token's detail page">
    Navigate to [actfun.xyz](https://actfun.fun) , find the graduated token you want, and click its card to open the detail page.
  </Step>
  <Step title="Select Buy in the swap panel">
    The swap panel on the right side of the page defaults to the **Buy** tab. Enter the amount of ARC you want to spend.
  </Step>
  ...
  <Step title="Confirm in MetaMask">
    Click **Buy**. MetaMask opens with the ARC amount as the transaction value. Click **Confirm** to submit. Tokens arrive in your wallet once the transaction confirms.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Review the estimated ARC output">
    The panel shows the estimated ARC you'll receive based on the current pool state.
  </Step>
  <Step title="Click Approve">
    If you haven't approved the launcher contract for this token yet, the panel shows an **Approve** button first. Click it and confirm the approval transaction in MetaMask. This authorises the contract to transfer your tokens when you sell.
  </Step>
  <Step title="Click Sell">
    Once the approval confirms, click **Sell** and confirm the second transaction in MetaMask. The contract transfers your tokens into the pool and sends ARC to your wallet.
  </Step>
</Steps>
